...
findMissingGenDJS = (K,J) -> (
    newGens0 = {}; --initialize a list that will contain all of the missing generators.
    orderingRing = QQ[flatten entries vars ring J, MonomialOrder => {Weights => join(toList(#newD:0), toList(#(transpose newD): -1)), Weights => join(toList(#newD:1), toList(#(transpose newD): 0))}, Degrees => join(toList(#newD:0), toList(#(transpose newD): 1)), Global=>false];  --Initialize a polynomial ring with the variables from J and in(I). So that elements are ordered, variables are weighted consecutively so that small y's and large x's are prioritized. Degrees established so degree is taken only with resect to the y's.
...
khovanskiiBasisDJS = (w, polyRing, algebraGens, limit, D) -> (
	...
	while (isSubset(J,K) == false) do ( --if the ideals are not yet equal
		...
		g = sub(findMissingGenDJS(K,J), S); --let g be the lowest-degree element of J but not K, then sub into the ring S.
		...
		newD = transpose( transpose newD | { ((exponents phi(g))_0)_{#transpose(D)..#transpose(D) + #D - 1} } ); --expend the matrix D by adding a new column corresponding to new variable
...
DJS = (L, DMat, limit) -> (  --input is ideal L, matrix D, and positive integer "limit"
	...
	newBasis_k = delete(0, khovanskiiBasisDJS(w_k,R,alggens_k,limit,D')); --pass previous list of algebra generators through khovanskii basis algorithm to add an additional generator, if needed. Delete any additional zeros if necessary.
